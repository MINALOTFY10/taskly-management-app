...
import { MoreHorizontal, CalendarDays, UserRound } from "lucide-react"

import { cn } from "@/lib/utils"
import { Button } from "@/components/ui/button"
import type { EpicRow } from "@/features/epics/types"
import EpicAvatar from "@/features/epics/components/epic-avatar"
...
export default function EpicCard({ epic }: EpicCardProps) {
  ...
  const cardStyle = cn(
    "group flex flex-col rounded-xl border-l-7 border-[#004E32] bg-card p-4 shadow-sm transition-colors hover:border-primary/20 hover:bg-accent/10 sm:p-5",
    statusLabel === "Done" && "bg-primary/5 border-0! hover:bg-primary/5"
  )
  ...
  return (
    <article className={cardStyle}>
      ...
      {/* ── Desktop: Assignee row with status badge on right ── */}
